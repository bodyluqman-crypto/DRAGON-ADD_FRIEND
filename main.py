...
[bَ][FFD700ً]3MK DRAGON

[FFFF50][b]/help
ㅤㅤㅤㅤㅤㅤㅤㅤ 

استمتع ب مميزات البوت """,uid
                            )
                        )
                else:
                    pass  
            ...
            if "1200" in data.hex()[0:4] and b"WELCOME TO ZIX and devx BOT" in data:
            	pass
            else:
	            ...
	            if "1200" in data.hex()[0:4] and b"/v" in data:

	                command_split = re.split("/v", str(data))
	                if len(command_split) > 1:
	                    player_id = command_split[1].split('(')[0].strip()
	            ...
	            if "1200" in data.hex()[0:4] and b"/help" in data:
	                ...
	                clients.send(
	                    self.GenResponsMsg(
		                        f"""
		                        [b][ffff50]⬇️️━━━━━━━━━━⬇️
		                        [CCFF00]welcome to ELMASRY DRAGON BOT[B][FFFF00]Ⓥ

[FFF8DC]Devloped by DRAGON 

[B][FF0009]command!

[ffff50]⬇️️━━━━━━━━━━⬇️

[B]•[ffffff] جميع اوامر فتح الفريق : [ff0050]
/َ3s
/َ5s
/َ6s
[ffffff]• فتح الفريق عبر المعرف :
[50ffff]/َsnd + uid

[ffffff]• دخول و خروج البوت عبر team code :
[0050ff]
/َenter + team code
/َsolo

[ffffff]• لاج عبر team code :
[50ff50] /َlag + team code

[ffffff]• لاج + بداء اجباري عبر team code :
[50ff50] /َfs + team code

[ffffff]• معرفة محظور او لا :
[c000ff] /َchk + uid

[ffffff]• معرفة الحساب في اي سيرفر : [c000ff] /َregion + uid
...
""", uid
	                ...
	                clients.send(
		                    self.GenResponsMsg(
		                        f"""
[B]• لزيادة عدد اللايكات :
[FF7F00] /َlike

[ffffff]• لمعرفة معلومات اللاعب :
[FF7F00] /َinfo

[FFFFFF]• لمعرفة قائد و حالة الفريق :
[50FFC0] /َs

[ffffff]• للحديث مع الذكاء الاصطناعي :
[50FFC0] /َai + tall

[ffffff]• لعمل اسبام داخل الروم :
[C06030] /َrm

[ffffff]• لعمل اسبام طلبات صداقة :
[C06030] /َspam

[ffffff]• لعمل اسبام انضمام عبر الايدي :
[ff50ff] /َx

[ffffff]• لزيادة 1000 زائر :
[ff50ff] /َv

[ff90ff] سوف يتم تطوير البوت واضافة مميزاة افضل❤️
...
[FFFF50]⬇️━━━━━━━━━━⬇️
	                ...
	                clients.send(
		                    self.GenResponsMsg(
		                        f"""
[C][B][50FFFF]admin DRAGON ELMASRY

[B][FFFF50]━━━━━━━━━━━━

[B][FFFFFF]INSTGRAM : [ff0050]/[50ffff]@laxhs_3s

[B][FFFFFF]TELEGRAM : [ff0050]/[50ffff]
@Z7F_DR

[FFFF50]━━━━━━━━━━━━

[B][FF0000]لشراء البوت يجب عليك التواصل مع المشرفين.  """,uid
	                    )
	                )
            ...
            if '1200' in data.hex()[0:4] and b'/lag' in data:
                try:
                    ...
                    if not command_parts:
                        clients.send(self.GenResponsMsg("[C][B][FF0000]Please enter the team code!.", uid))
                        continue
            ...
            if '1200' in data.hex()[0:4] and b'/fs' in data:
                try:
                    ...
                    split_data = re.split(rb'/fs', data)
                    command_parts = split_data[1].split(b'(')[0].decode().strip().split()
            ...
            if "1200" in data.hex()[0:4] and b"/enter" in data:
                try:
                    # تقسيم البيانات القادمة بعد الأمر
                    split_data = re.split(rb'/enter', data)
                    command_parts = split_data[1].split(b'(')[0].decode().strip().split()
...
for thread in threads:
    thread.join()
# غير توكن حسابك  هنا لحاااسين    
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="4197963977", password="8D237E619C69E6533414496270E8E3F83735AB902D79604FA6AD1A2253ACE297")
        client_thread.start()
    except Exception as e:
        logging.error(f"Error occurred: {e}")
        restart_program()
        
# اعمل اداة ترجى تشغل البوت كل دقيقتين لانه في وضع خمول 
